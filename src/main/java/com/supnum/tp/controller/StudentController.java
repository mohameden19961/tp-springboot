...
public class StudentController {
    ...
    @PostMapping
    public Student create(@RequestBody StudentDTO dto) { return studentService.create(dto); }

    @GetMapping("/{id}")
    public Student getById(@PathVariable @NonNull Long id) { return studentService.getById(id); }

    @DeleteMapping("/{id}")
    public void delete(@PathVariable @NonNull Long id) { studentService.delete(id); }
    ...
    @GetMapping("/search")
    public List<Student> search(@RequestParam String name) { return studentService.search(name); }

    @GetMapping("/page")
    public Page<Student> getAllPaged(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size) {
        return studentService.getAllPaged(page, size);
    }
